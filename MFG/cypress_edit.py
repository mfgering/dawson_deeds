...
class Sheet_Editor(object):
    ...
    def do_sheet(self, desktop):
        ...
        try:
            ...
            if not model.Sheets.hasByName("cypress_deeds"):
                idx = model.Sheets.getCount()
                model.Sheets.insertNewByName("cypress_deeds", idx)
            sheet = model.getSheets().getByName("cypress_deeds")
                    ...
                    for col in range(0, len(row_v)):
                        ...
                        try:
                            if col == 4: # deed_date
                                ...
                                cell.String = col_val
                                if row > self.header_row_num:
                                    hlink = f"http://services.wakegov.com/realestate/Account.asp?id={col_val}"
                                    self.do_hyperlink_fmt(cell, col_val, hlink)
                            elif col == 8: # photo
                                cell.String = col_val
                                if row > self.header_row_num:
                                    hlink = f"https://services.wake.gov/realestate/Photo.asp?id={row_v[7]}"
                                    self.do_hyperlink_fmt(cell, col_val, hlink)
                            else:
                                cell.String = col_val
    ...
    def do_hyperlink_fmt(self, cell, text_val, url):
        self.ctlr.select(cell)
        props = [PropertyValue(Name="Hyperlink.Text", Value=text_val),
...
g_exportedScripts = (update_deeds_sheet, init_debug)